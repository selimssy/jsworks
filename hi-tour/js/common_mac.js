...
let empty = 0
let overs = [empty, isOver1, isOver2, isOver11, isOver22, isOver111, isOver222, isOver1111, isOver2222, isOver11111, isOver22222]
...
$(document).ready( function() {
	...
	for(let i = 1; i <= 4; i++){
		$('.openAll' + i).mouseover(function(){
			if ( parseInt($('header').css('width')) > 800 ) $('.gnb_depth2_' + i).fadeIn('fast');
			overs[2*i - 1] = true;
		});	
		$('.openAll' + i).focus(function(){
			if ( parseInt($('header').css('width')) > 800 ) $('.gnb_depth2_' + i).fadeIn('fast');
			overs[2*i - 1] = true;
		});
		...
		$('.gnb_depth2_' + i + 'li:last-child a').blur(function(){
			overs[2*i - 1] = false;
			setTimeout("goHide" + i + "()",200);
		});	
		...
		$('.gnb_depth2_' + i).focus(function(){
			overs[2*i] = true;			
		});	
		$('.gnb_depth2_' + i).mouseout(function(){
			overs[2*i] = false;
			setTimeout("goHide" + i + "()",200);
		});	
		$('.gnb_depth2_' + i).blur(function(){
			overs[2*i] = false;
			setTimeout("goHide" + i + "()",200);
		});	
	}
